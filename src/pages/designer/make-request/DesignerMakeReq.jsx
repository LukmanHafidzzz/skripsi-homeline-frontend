...
export default function DesignerMakeReq() {
        ...
        if (selected !== 'Semua') {
            if (selected === 'Belum Dibuat') {
                filtered = filtered.filter(
                    item => !item.house.design_request
                );
            } else {
                filtered = filtered.filter(
                    item =>
                        item.house.design_request &&
                        item.house.design_request.request_status === selected
                );
            }
        }
                        ...
                        filteredHouses.map((houseProcess, index) => (
                            <tr key={houseProcess.house.id}>
                                ...
                                <td className="text-center">
                                    {houseProcess.house.design_request ? (
                                        <span className={`badge w-100 py-2 ${houseProcess.house.design_request?.request_status === 'Approved'
                                            ? 'bg-success'
                                            : houseProcess.house.design_request?.request_status === 'Rejected'
                                                ? 'bg-danger'
                                                : 'bg-warning'
                                            }`}>
                                            {houseProcess.house.design_request?.request_status}
                                        </span>
                                    ) : (
