...
export default function SurveyNeedRevDetail() {
    ...
    const isFacilityFilled = generalFacilities.some(f =>
        f.name && f.type_id && f.latitude && f.longitude && f.maps
    );
    const isFormValid = file || link || isFacilityFilled;

    const handleSubmit = async (e) => {
        e.preventDefault();
        setButtonLoading(true);

        if (!file && !link && !isFacilityFilled) {
            Swal.fire(
                'Warning',
                'Minimal isi salah satu: file survey, link dokumentasi, atau fasilitas umum.',
                'warning'
            );
            return;
        }
        ...
        try {
            const res = await axios.put('https://skripsi-homeline-backend.vercel.app/api/surveyor/rev-house-survey', formData, {
                withCredentials: true,
                headers: {
    ...
    return (
        <>
            <Container>
                ...
                <Row>
                    <Col className="fw-bold fs-3 p-0 mt-5" data-aos="fade-up" data-aos-duration="800">
                        REVISI
                    </Col>
                </Row>
